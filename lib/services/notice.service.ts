...
export class NoticeService {
	...
	async createNotice(
		...
		if (validated.targetAudience === NOTICE_TARGETS.SPECIFIC_BUILDING) {
			const targetBuildingId = validated.targetBuildingId;
			if (!targetBuildingId) {
			...
			if (ctx.role === "manager") {
				await this.validateManagerBuildingAccess(ctx.userId, targetBuildingId);
			}
		}

		if (validated.targetAudience === NOTICE_TARGETS.SPECIFIC_FLAT) {
			const targetFlatId = validated.targetFlatId;
			if (!targetFlatId) {
				throw new ValidationError([
					{
						field: "targetFlatId",
	...
	async updateNotice(
		...
		const effectiveTargetAudience =
			validated.targetAudience ?? existing.targetAudience;
	...
	async getNotice(
		...
		if (
			!this.isVisibleToUser(ctx, row as unknown as typeof notices.$inferSelect)
		) {
			throw new NotFoundError("Notice");
		}

		return this.mapToResultWithAuthor(row);
	}
			...
			throw new ValidationError([
				{
					field: "isPinned",
					message:
						"Maximum of 5 pinned notices per target audience scope reached",
					rule: "max_pinned",
				},
			]);
		}
