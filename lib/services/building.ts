...
export class BuildingService {
	...
	constructor(db: Database, auditLogger: IAuditLogger, r2?: IR2Client) {
		this.db = db;
	...
	async createBuilding(
		...
		return await this.db.transaction(async (tx) => {
			...
			if (!fullBuilding) {
				throw new Error("Failed to retrieve building after creation");
			}
			return fullBuilding;
		});
	}
	...
	async updateBuilding(
		...
		return await this.db.transaction(async (tx) => {
			...
			if (!fullBuilding) {
				throw new Error("Failed to retrieve building after update");
			}
			return fullBuilding;
		});
	}
