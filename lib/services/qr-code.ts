	...
	async generateQrCode(slug: string, options?: QrCodeOptions): Promise<Buffer> {
		...
		for (let y = 0; y < png.height; y++) {
			for (let x = 0; x < png.width; x++) {
				...
				output.data[dstIdx] = png.data[srcIdx] ?? 0;
				output.data[dstIdx + 1] = png.data[srcIdx + 1] ?? 0;
				output.data[dstIdx + 2] = png.data[srcIdx + 2] ?? 0;
				output.data[dstIdx + 3] = png.data[srcIdx + 3] ?? 0;
			}
		}
