...
export class MaintenanceService {
	...
	async createRequest(
		...
		if (ctx.role === "renter") {
			...
			renterId = renter.id;
			flatId = ctx.assignedFlatId || null;
			...
			// Owner or Staff: Must provide buildingId via input data
			if (!validated.buildingId) {
				throw new ValidationError([
					{
						field: "buildingId",
						message:
							"buildingId is required for staff creating maintenance requests",
						rule: "required",
					},
				]);
			}
			...
			// Verify building access for non-owners
			if (ctx.role !== ROLES.OWNER && ctx.assignedBuildingIds) {
				if (!ctx.assignedBuildingIds.includes(buildingId)) {
	...
	async listRequests(
		...
		if (ctx.role === "renter") {
			...
			conditions.push(
				eq(maintenanceRequests.flatId, context.activeContract.flatId),
			);
		}
