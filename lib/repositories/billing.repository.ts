...
export class BillingRepository extends BaseRepository {
	...
	list(
		...
		if (
			...
			conditions.push(
				inArray(bills.flatId, this.getAssignedFlatIdsSubquery(scope)),
			);
		}
	...
	private getAssignedFlatIdsSubquery(scope: ScopeContext) {
		return this.db
			.select({ id: flats.id })
			.from(flats)
			.where(inArray(flats.buildingId, scope.assignedBuildingIds ?? []));
	}
	...
	create(data: typeof bills.$inferInsert, tx?: DatabaseOrTransaction) {
		const client = this.txOrDb(tx);
	...
	insertLineItems(
		data: (typeof billLineItems.$inferInsert)[],
		tx?: DatabaseOrTransaction,
	) {
		const client = this.txOrDb(tx);
