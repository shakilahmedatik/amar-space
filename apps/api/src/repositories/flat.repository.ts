...
export class FlatRepository {
   ...
   * Creates a new flat.
   */
  async create(data: typeof flats.$inferInsert, tx?: Database) {
    const client = tx ?? this.db
   ...
   */
  async delete(id: string, tx?: Database) {
    const client = tx ?? this.db
   ...
   */
  async findSlugByFlatId(flatId: string, tx?: Database) {
    const client = tx ?? this.db
   ...
   * Creates a flat slug.
   */
  async createSlug(data: typeof flatSlugs.$inferInsert, tx?: Database) {
    const client = tx ?? this.db
