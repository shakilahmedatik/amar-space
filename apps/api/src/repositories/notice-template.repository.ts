import { type Database, noticeTemplates } from '@repo/db'
...
export class NoticeTemplateRepository extends BaseRepository {
  ...
  create(data: typeof noticeTemplates.$inferInsert, tx?: Database) {
    const client = this.txOrDb(tx)
  ...
  delete(id: string, ownerAccountId: string, tx?: Database) {
    const client = this.txOrDb(tx)
    return client
